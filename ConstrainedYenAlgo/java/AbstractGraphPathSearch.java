...
public abstract class AbstractGraphPathSearch<V extends Vertex, E extends Edge<V>>
    ...
    @Override
    public Result<V, E> search1(Graph<V, E> graph, V src, V dst,
                               EdgeWeigher<V, E> weigher, int maxPaths, List<E> whiteEdge, List<V> whiteVertex) {
        checkArguments(graph, src, dst);

        return internalSearch1(graph, src, dst,
                weigher != null ? weigher : new DefaultEdgeWeigher<>(),
                maxPaths, whiteEdge, whiteVertex);
    }

    protected abstract Result<V, E> internalSearch(Graph<V, E> graph, V src, V dst,
                                          EdgeWeigher<V, E> weigher, int maxPaths);

    protected abstract Result<V, E> internalSearch1(Graph<V, E> graph, V src, V dst,
                                                   EdgeWeigher<V, E> weigher, int maxPaths,
                                                    List<E> whiteEdge, List<V> whiteVertex);
}
